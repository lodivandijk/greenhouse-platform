...
PostgreSQL, 30 tables via Flyway migrations V1–V32 (`backend/src/main/resources/db/migration/`), `spring.jpa.hibernate.ddl-auto=validate` (schema changes only happen through a migration, never Hibernate auto-DDL):
...
- **`crop_monitoring_profile`** (V10, V30) — versioned per-crop interpretation: preferred temperature range, excursion/recovery durations, soil strategy (`EVENLY_MOIST`/`DRY_BETWEEN_WATERING`), index thresholds, `soil_monitoring_mode` (`SENSOR`/`MANUAL`, V30) and `soil_moisture_band` (`HIGH`/`MEDIUM`/`LOW`, V32). A change creates a new version so historical assessments keep referencing the one that produced them. Seeded for the six herbs by V27; V30 adds Tarragon's version 2 as `MANUAL` and retains version 1, disabled, as history.
...
- **`daily_briefing_snapshot`** (V28, V33) — one immutable structured briefing per greenhouse day **per edition** (`MORNING` 06:00, `EVENING` 19:00). Deliberately *not* unique in the database: regeneration creates a new version linked via `supersedes_snapshot_id`, and "one per scheduled occurrence" is enforced in `DailyBriefingService`. See ADR-033.
...
- **REST authentication** (`ApiAuthenticationFilter`, ADR-025) classifies every non-MCP request into three groups. Reads, the dashboard and `/actuator/health` are open on the local network by explicit choice. Every `POST`/`PUT`/`PATCH`/`DELETE` outside device ingestion requires the admin token (`GREENHOUSE_ADMIN_TOKEN`). `/api/v1/heartbeats` and `/api/v1/observations` require a **per-device** token, and `DeviceIdentityGuard` rejects an authenticated device that claims another device's id in its payload, so one compromised probe cannot forge readings for the rest. Enforcement fails closed and startup fails fast when a required token is missing.
- **Outcome evaluation measures against a pre-action baseline** (ADR-026): the latest reading at or before the work was completed, compared with the peak reading inside the stored `[evaluateAfter, windowEnd]` window. No baseline or no readings in the window yields `INCONCLUSIVE` rather than a guess. Outcomes recorded before 2026-09-02 used a different, incorrect comparison and should not be treated as evidence.
- **Idempotent MCP writes report ownership** (ADR-027): `reserve()` returns `ACQUIRED`/`ALREADY_COMPLETED`/`IN_PROGRESS`/`CONFLICT`, and only the acquiring caller runs the action. Exactly-once for concurrent and sequential retries; at-least-once for a caller that crashes mid-request, whose reservation is taken over after five minutes rather than wedging that key forever.
- Four schedulers run, on deliberately different cadences: `GreenhouseEvaluationScheduler` (1 min — twin, assessments, and care-loop correlation on one clock reading), `OutcomeEvaluationScheduler` (5 min — evaluation windows are hours-scale and per-execution), `DailyBriefingScheduler` (ticks every minute and generates whichever edition is due — `morning-at` 06:00 and `evening-at` 19:00 `Europe/London` by default), and `NotificationSweepScheduler` (5 min — notification policy then delivery, plus a startup sweep).
- The briefing schedule has exactly one source of truth: `morning-at` / `evening-at` + `zone`. There is no separate cron property — changing a time genuinely changes when it fires. Startup recovery re-checks the same due condition, so a restart before the configured hour generates nothing rather than producing a "daily briefing" at midnight.
- **An edition is generated only while it is still the current one** — due, missing, and not yet overtaken by the next edition's time. A restart at 20:00 therefore produces the evening briefing and leaves the morning one un-generated, rather than sending two at once. The consequence is accepted deliberately: a missed edition is never back-filled once its successor is due, so a day can honestly end with one briefing rather than a retrospective invention. Each edition's window starts at the previous edition's scheduled time, which is what lets the evening briefing report what changed while the reader was out. See ADR-033.
...
- A crop's **soil moisture band** (`HIGH`/`MEDIUM`/`LOW`) decides its dry and wet thresholds; the resolved numbers are stored on the profile version alongside the band, so the assessment rule reads plain thresholds and a historical assessment still shows the values that produced it even after a band is later redefined. Adding a band is a code change plus a migration, deliberately (ADR-028).
- A crop's `soil_monitoring_mode` decides whether it is sensor-assessed at all. `SENSOR` (the default, and every crop but Tarragon) keeps the four-step assignment/calibration/staleness/threshold check. `MANUAL` means the crop is deliberately tended by eye: `CropSoilMoistureAssessmentRule` skips it entirely and raises none of `CROP_SENSOR_NOT_ASSIGNED`, `CROP_SENSOR_CALIBRATION_REQUIRED`, `CROP_SENSOR_DATA_STALE`, or the moisture thresholds. **This is not a claim that the soil is fine** — it is unknown, and the briefing and email both say so explicitly rather than omitting the crop. A deliberate absence of a probe is correspondingly not counted as a data-quality gap, since a gap means "we tried to measure and could not". See ADR-024.
...
An MCP server runs inside the same process (`com.greenhouse.mcp`), reachable at `POST /mcp` using the Streamable HTTP transport from the official MCP Java SDK (`io.modelcontextprotocol.sdk`, hand-wired — not Spring AI's Boot starter; see ADR-015 for why). Twenty-nine tools are exposed:
...
Write:  create_crop, update_crop, create_goal, record_harvest, record_crop_observation, record_action,
        ...
        set_crop_soil_monitoring_mode, set_crop_soil_moisture_band
...
`set_crop_soil_monitoring_mode` switches a crop between `SENSOR` and `MANUAL`, creating a new monitoring-profile version and preserving the previous one. It requires an `idempotencyKey` and a `rationale`, and carries the same explicit-human-confirmation notice as the care-loop response tools — turning off a crop's soil warnings removes a real safety net and is not the agent's call to make alone. `get_crop_monitoring_profile_history` returns every version, so "why is this crop not being sensor-assessed, and who decided that?" is answerable.

`set_crop_soil_moisture_band` sets how thirsty a crop is judged to be (`HIGH` 50/none, `MEDIUM` 40/85, `LOW` 30/75), which resolves to its moisture thresholds. Per crop, not per species. There is deliberately no tool taking a raw threshold number — a hand-picked number is one nobody can justify later. See ADR-028.
...
- **Reads and the dashboard are unauthenticated on the local network, by explicit decision** (ADR-025). Writes, deletes and device ingestion are not. Anyone on the house WiFi can read greenhouse conditions and crop names; nobody can change anything without a token.
- **Device tokens travel in cleartext.** The ESP32 speaks plain HTTP and there is no TLS anywhere inside the house, so a passive WiFi capture yields a device credential. This reduces the attack surface from "anyone who can reach the port" to "anyone who can capture traffic", which is a real improvement and explicitly not the end state — TLS on the Pi plus certificate handling on the ESP32 is a much larger change.
...
- **Local dev and the Pi have independently-managed database credentials, MCP tokens, admin tokens and device tokens** (not shared, not centrally rotated). Rotating a device token requires a firmware flash.
- **Assessment reconciliation and care-loop correlation are separately committed**, not one transaction. If correlation fails after reconciliation succeeds, an immediately-actionable condition can stay uncorrelated until the next minute's tick. This is accepted eventual consistency, not atomicity.
...
- **A `MANUAL` crop has no soil safety net at all.** Setting the mode to quieten an alert on a crop that really does have a failing probe would stop the platform ever warning about it. The required `rationale` and the preserved version history make the change attributable, but nothing prevents it — a mode that could be overridden would not do what it says.
- **A probe assigned to a `MANUAL` crop produces readings but no assessment.** That is the honest reading of the configuration, but it may surprise someone who wires a probe and expects alerts without also changing the mode back to `SENSOR`.
...
- **The briefing's summary paragraph is written by a language model** (ADR-029, ADR-030), off by default and enabled per deployment. It composes once a day into the immutable snapshot, is given only a structured fact sheet built from that same snapshot, and decides nothing — assessments, thresholds and care loops are all computed before it runs. There is no second writer: if the model is unconfigured or unreachable the briefing carries an explicit `unavailableReason` and no paragraph, while its readings, trends and warnings are unaffected. The notification renderer stays deterministic and renders the stored text, so no model sits in the delivery path.
...
- **Two notification channels: email and push (ntfy).** Each port declares its own recipient, content format and which intent types it carries, so a phone can want less than an inbox. **Email carries briefings only** — warnings, reminders and recoveries go to the phone alone (ADR-033), and the complete record of what was decided and delivered lives in `notification_intent` / `notification_delivery_event`, queryable via `get_notification_history`. The briefing email is deliberately short: summary, what needs action, one line per crop, unmeasured things, and the moisture caveat. The push carries a model-written headline or a one-line alert. A warning in either channel states what was measured, on which crop, against what it should have been — `greenhouse-01 Crop 12 Oregano temperature 25C, above its preferred maximum of 24C` — built from the evidence the assessment engine recorded when it raised the assessment. The moisture index is never written bare or as a percentage: it always carries its scale (`soil 43 of 100, at or below its dry line of 50`), since it is a position between one probe's own references (ADR-031, ADR-032). There is no inbound path: nothing can be actioned by replying to a message.

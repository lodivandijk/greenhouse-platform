...
- **Outcome evaluation measures against a pre-action baseline** (ADR-026): the latest reading at or before the work was completed, compared with the peak reading inside the stored `[evaluateAfter, windowEnd]` window. No baseline or no readings in the window yields `INCONCLUSIVE` rather than a guess. Outcomes recorded before 2026-08-31 used a different, incorrect comparison and should not be treated as evidence.

...
- **Soil moisture is live but uncalibrated.** The ESP32 now sends `soilMoisture` (raw ADC only) alongside every BME280 observation, on the same 60s cycle, and the Pi persists each reading to `soil_moisture_reading` (V9) — confirmed live for `soil-01`..`soil-04` (GPIO34/33/32/35; `soil-05`/GPIO36 is wired but not yet enabled in `SoilSensors::ACTIVE_COUNT`), per `docs/architecture/soil-moisture-sensor-integration-v2-spec.md` and ADR-018. No calibration, moisture percentage, dry/wet assessment, twin exposure, or dashboard presentation exists yet — those remain deliberately deferred (spec §8–9).

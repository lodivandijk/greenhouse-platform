...
- **The briefing's summary paragraph may be written by a language model** (ADR-029), off by default. It composes once a day into the immutable snapshot, is given only a fact sheet built from that same snapshot, and decides nothing — assessments, thresholds and care loops are all computed before it runs. Deterministic text is always computed first and is used whenever the model is disabled or unreachable; every briefing records which source wrote it. The notification renderer itself stays deterministic and renders the stored text, so no model sits in the delivery path.
- **The summary is not testable the way the structured briefing is.** The deterministic narrator's honesty properties are covered by tests; the model's are covered by its prompt and by the fact that nothing acts on its output. That is a weaker guarantee, deliberately accepted.

...
- `CropService.deleteCrop`'s existing scoped-delete guard (ADR-016) is extended to also check `ActionRepository.existsByCropId`, alongside the pre-existing goal/harvest/observation checks. Without this, a crop with recorded actions but no goals/harvests/observations would pass the guard and then fail with a raw FK-violation on `action.crop_id REFERENCES crop(id)` instead of the clean `DomainValidationException` ADR-016 established — this was caught during live verification of this change, not assumed.
